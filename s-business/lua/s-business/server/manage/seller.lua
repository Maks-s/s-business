SA.Business:AddEvent( "AddSeller", function( pPlayer, tblInfos )
	...
	if not tblInfos[ 'SellerName' ] then return end
	if not tblInfos[ 'SellerModel' ] then return end

	if not SA.Business.List[ tblInfos[ 'Business' ] ] then return end

	if not SA.Business:HasPermission( tblInfos[ 'Business' ], pPlayer, "Manage Sellers" ) then
	...
	if not SA.Business.SellersList then
		SA.Business.SellersList = {}
	end	

	if not SA.Business.SellersList[ tblInfos[ 'Business' ] ] then
...
SA.Business:AddEvent( "DeleteSeller", function( pPlayer, tblInfos )
	...
	if not SA.Business.List[ tblInfos[ 'Business' ] ][ 'Sellers' ][ tblInfos[ 'SellerName' ] ] then
...
SA.Business:AddEvent( "AddContentToSeller", function( pPlayer, tblInfos )
	...
	if not tblInfos[ 'Price' ] then return end
	...
	if not SA.Business.List[ tblInfos[ 'Business' ] ][ 'Sellers' ] then return end
	if not SA.Business.List[ tblInfos[ 'Business' ] ][ 'Sellers' ][ tblInfos[ 'SellerName' ] ] then
		return SA.Business:AddNotify( pPlayer, SA.Business:GetLanguage( "InvalidSeller" ), SA.Business.Red, 5 )
	end

	if not SA.Business.List[ tblInfos[ 'Business' ] ][ 'Sellers' ][ tblInfos[ 'SellerName' ] ][ 'Contents' ] then SA.Business.List[ tblInfos[ 'Business' ] ][ 'Sellers' ][ tblInfos[ 'SellerName' ] ][ 'Contents' ] = {} end
	...
	if not SA.Business.SellersContents[ tblInfos[ 'Content' ] ] then return end
...
SA.Business:AddEvent( "BuySeller", function( pPlayer, tblInfos )
	...
	if not tblInfos[ 'Cart' ] then return end
	...
	if not SA.Business.List[ Ent:GetSellerBusiness() ][ 'Sellers' ][ Ent:GetSellerName() ] then
	...
	for k,v in pairs( tblInfos[ 'Cart' ] ) do
		if not SA.Business.SellersContents[ k ] then NotExist = k break end 
		if not tblSeller[ "Contents" ][ k ] then NotExist = k break end
	...
	for k,v in pairs( tblInfos[ 'Cart' ] ) do
		if not tblSeller[ "Contents" ][ k ] then continue end
